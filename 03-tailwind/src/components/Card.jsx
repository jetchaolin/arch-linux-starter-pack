  ...
  return (
    <>
      <div
        ...
        class="max-w-[500px] max-h-[500px] flex flex-col justify-center items-center overflow-hidden"
      >
        {!loading ? (
          <h1
            id={`title-id-${cardData.id}`}
            className={`title-${cardData.title[0]}`}
            class="transition duration-300 hover:scale-[1.05] text-2xl text-[var(--primary-color)] my-3"
          >
            {cardData.title[0]}
          </h1>
        ) : (
        ...
        {!loading ? (
          <img
            ...
            loading="lazy"
            class="transition duration-300 hover:scale-[1.05] hover:shadow-2xl hover:shadow-black/50 block border border-[var(--primary-color)] rounded-lg mx-4 mb-8 w-[300px] h-[170px] object-cover object-center"
          />
        ) : (
          <Skeleton
            animation="wave"
